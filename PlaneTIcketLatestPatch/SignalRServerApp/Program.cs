...
// HTTP endpoint to receive notifications from WebSocket server
app.MapPost("/notify-flight-change", async (HttpContext context) =>
{
    try
    {
        ...
        var hubContext = context.RequestServices.GetRequiredService<Microsoft.AspNetCore.SignalR.IHubContext<FlightHub>>();
...
Console.WriteLine("   🔔 Notification endpoint: /notify-flight-change (WebSocket серверээс)");
